...
The implemented Phase 2 slice is narrower than this phase definition. It
provides bounded production Codex `0.144.4` app-server discovery, privacy-safe
normalization, atomic complete-scan reconciliation, configured-location
assignment, retained no-refresh reads, and canonical `snapshot` / `list` JSON.
Provider failure is structured degradation and never turns a partial scan into
authoritative absence. Snapshot session candidates are bounded at the database
read and canonical UTF-8 serialization layers; explicit truncation counts are
reported without deleting retained rows. The evidence and exact remaining
boundary are recorded in
[`docs/phase-2-validation.md`](phase-2-validation.md).

Claude discovery, Codex and Claude hooks, runtime/process/tmux liveness,
normalized live-state transitions, and hook installation remain unimplemented
parts of Phase 2.
...
The repository uses Hatchling to build reproducible Python wheels and source
distributions. Artifact contents are explicit and verified: runtime wheels
contain the package, migrations, entry point, metadata, and MIT license; source
archives add the documented build inputs, design, validation records, and
project metadata without shipping tests, provider fixtures, repo-only smoke
tooling, caches, databases, prompts, or credentials. CI fixes
`SOURCE_DATE_EPOCH`, builds twice, and requires byte-identical artifacts before
isolated install smoke tests.

This resolves the build and artifact-shape decision. The public release channel
and provider-hook installation workflow remain separate open decisions.
...
Choose the public release channel and whether downstream system packages or
standalone archives accompany the verified Python artifacts. Separately,
choose whether provider-hook configuration is generated by an explicit install
command or managed entirely by dotfiles. Hook installation must remain
explicit and must preserve existing provider hooks.
